# Tiny CV Blog Topic Plan

## Editorial Thesis

Tiny CV helps job seekers put their best self forward with clear one-page resumes, role-specific versions, public CV links, and careful agent-assisted workflows that do not invent facts.

The blog should feel useful before it feels strategic. Prefer practical posts someone can apply during an active job search.

## Audience Priority

1. Job seekers updating or tailoring a resume.
2. Candidates who want a clean public CV link or PDF.
3. People using agents to draft, review, or publish career materials.
4. Developers integrating Tiny CV only when the topic has clear pull.

## Topic Backlog

### Resume Writing

- How to write a one-page resume that does not feel cramped
- Resume summary examples for career switchers
- What to cut from a resume when it is too long
- How to turn rough work notes into resume bullets
- How to write resume bullets without inventing metrics
- The resume sections that matter most for technical roles

### Job Search

- How to tailor a resume for each role without rewriting everything
- Public resume link vs PDF: when to send each one
- How to keep multiple resume versions organized
- What to do when a job description is vague
- How to update your resume quickly before a warm intro
- How to make a resume readable on mobile

### Career Materials

- What belongs on a public CV link
- How to make a resume, LinkedIn, and personal site tell the same story
- A simple pre-send checklist for resumes
- How to explain a non-linear career path
- How founders should translate company-building work into a resume

### Agents

- How to use an AI agent to improve your resume without inventing facts
- What agents should ask before drafting your CV
- How to review an AI-edited resume before publishing
- The safest way to use AI for role-specific resume versions
- How to hand off a Tiny CV edit link after an agent draft

### Developer API

- How agents can publish a resume link without building a resume renderer
- When to use Tiny CV's API vs the no-account Agent Finish endpoint
- How to validate resume markdown before publishing

## Preferred Formats

- **Protocol / How-To:** Best for practical workflows job seekers can follow immediately.
- **Decision Framework:** Best for "should I..." questions like public link vs PDF, AI edits vs manual edits, or what to cut.
- **Comparison:** Best for product/workflow comparisons and high-intent search.
- **Research Deep-Dive:** Use sparingly for contested claims like ATS myths or recruiter screening behavior.
- **Standard Article:** Use only when the topic does not need a stronger structure.

## Avoid

- Generic "10 resume tips" posts.
- Fear-based ATS sludge.
- Unsupported recruiter-stat claims.
- Posts that imply Tiny CV can get someone hired by itself.
- Overly developer-first API content unless intentionally requested.
- AI hype that ignores verification, consent, or user review.

## Open Questions

- Which job-seeker segment should get the first deep cluster: engineers, founders/operators, designers, or sales?
- Should Tiny CV publish example-specific posts by template type?
- Should public CV link content stay job-seeker-oriented or include more creator/founder positioning?
